...
const MenuItems = ({isMobile, active, setActive}) => {
    ...
    return(
        <ul className={`list-none flexCenter flex-row ${isMobile && 'flex-col h-full'}`} style={{marginLeft:'20px'}}>
            {['Explore NFTs', 'Listed NFTs', 'My NFTs'].map((item, i) => (<li key={i} onClick={() => {setActive(item)}} 
            className={`flex flex-row items-center font-poppins font-semibold text-base dark:hover:text-white hover-text-white mx-3 ${active === item ? 'dark:text-white' : 'dark:text-nft-gray-3 text-nft-gray-2'}`}>
      ...
      <nav className='flex-between w-full fixed z-10 p-4 flex-row border-b dark:bg-nft-dark bg-white dark:border-nft-black-1 border-nft-gray-1'>
        ...
        <div className='flex flex-initial flex-row justify-end'>
            <div className='flex items-center mr-2' >
                <input className='checkbox' type='checkbox' id='checkbox' onChange={() => setTheme(theme === 'light' ? 'dark' : 'light')} />
            ...
            <div className='hidden md:flex ml-2'>
                {isOpen ? ( <Image src={images.cross} objectFit='contain' width={25} height={25} alt='menu' onClick={() => setisOpen(true)} className={theme === 'light' && 'filter invert'}/> )
                 : 
